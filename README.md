...
### Задание 1
#### Условие: 
Подготовить минимальный код на языке C с подключением модуля 
работы с матрицами. Создать матрицу 4x4, заполнить ее значениями 1-16, 
запустить код в отладчике gdb и осуществить dump памяти с данной матрицей в файл.
...
### Задание 2
#### Условие:
Создать в вашем репозитории дополнительную ветку fix, 
в новой ветке осуществить унификацию всех типов (uint64_t заменить на size_t).
Влить новую ветку в старую по шаблону rebase, то есть с переписыванием истории.
...
На новой ветке были заменены типы <i>uint64_t</i> на <i>size_t<i/>
в [исходном](src/matrix/matrix.c) и [заголовочном](src/matrix/matrix.h) файлах библиотеки.

**Добавление изменений и создание коммита**
```sh
git add src/
git commit -m "Replace data type for matrix size"
git push origin fix
```

**Слияние веток по шаблону rebase**
```sh
git checkout main
git rebase fix
